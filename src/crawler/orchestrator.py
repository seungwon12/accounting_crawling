...
워크플로우 (crawl_standard):
    ...
    5. 문단 후처리
       ...
       - 자기 기준서 내 문단 참조에 standard_number 채우기 (data-target-std=null인 자기참조)
       - 교차참조 범위(~) 전개
    6. 교차참조/QnA 인덱스 생성
    7. JSON 저장 + 체크포인트 기록
"""
...
from src.models import Paragraph, QnAReference, Standard, TocItem
...
async def crawl_standard(
    ...
    # (data-target-std가 null인 자기참조 → current standard_id로 채움)
